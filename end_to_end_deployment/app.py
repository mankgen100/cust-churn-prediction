...
def churn_prediction(tenure, citytier, warehousetohome, gender, hourspendonapp, numberofdeviceregistered, satisfactionscore, maritalstatus, numberofaddress, complain, orderamounthikefromlastyear, couponused, ordercount, daysincelastorder, cashbackamount):
    with open('E:\customer-churn-prediction-application-main\end_to_end_deployment\models\churn_prediction_model.pkl', 'rb') as f:
        model = pickle.load(f)

    with open("E:\customer-churn-prediction-application-main\end_to_end_deployment\models\columns.json", "r") as f:
...
def index_page():
    if request.method == 'POST':
        ...
        data = {
            'prediction': pred,
            'predict_probabality': f"{round(output_probab * 100, 2)}%"
        }
